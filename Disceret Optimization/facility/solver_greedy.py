...
def greedy_initial_solution(facility_count,customer_count,facilities_capacity,customers_demand,distance_matrix):
...
def solve_it(input_data):
    ...
    for i in setup_cost_sorted:
        for j in range(customer_count):
            if solution[j] == i:
                facility_count_index = [k for k in range(facility_count) if k not in unused]
                min_cost = 1e9
                for k in facility_count_index:
                    if distance_matrix[k][j] < min_cost:
                        min_cost = distance_matrix[k][j]
                        solution[j] = k
